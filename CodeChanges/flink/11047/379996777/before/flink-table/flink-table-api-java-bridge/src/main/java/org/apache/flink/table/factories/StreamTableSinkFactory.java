...
public interface StreamTableSinkFactory<T> extends TableSinkFactory<T> {
	...
	/**
	 * Creates and configures a {@link StreamTableSink} based on the given
	 {@link Context}.
	 *
	 * @param context context of this table sink.
	 * @return the configured table sink.
	 */
	default StreamTableSink<T> createStreamTableSink(Context context) {
		return createStreamTableSink(context.getTable().toProperties());
	}
	...
	default TableSink<T> createTableSink(Context context) {
		return createStreamTableSink(context);
	}
}
